...
async function getFirebaseDoc(collection, docId, env) {
  ...
  console.log(`FETCHING FIREBASE DOC: ${url}`);
  const res = await fetch(url);
  if (!res.ok) {
    const errText = await res.text();
    console.error(`FIREBASE GET DOC ERROR (${res.status}):`, errText);
    return null;
  }
  const data = await res.json();
...
async function setFirebaseDoc(collection, docId, data, env) {
  ...
  const url = `https://firestore.googleapis.com/v1/projects/${env.FIREBASE_PROJECT_ID}/databases/(default)/documents/${collection}/${docId}`;
  console.log(`SETTING FIREBASE DOC: ${url}`);
  const res = await fetch(url, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
  ...
  if (!res.ok) {
    const errText = await res.text();
    console.error(`FIREBASE SET DOC ERROR (${res.status}):`, errText);
  }
}
...
async function getFirebaseCollection(collection, env) {
  ...
  console.log(`FETCHING FIREBASE COLLECTION: ${url}`);
  const res = await fetch(url);
  if (!res.ok) {
    const errText = await res.text();
    console.error(`FIREBASE GET COLLECTION ERROR (${res.status}):`, errText);
    return [];
  }
  const data = await res.json();
...
async function sendMessage(chatId, text, env, replyMarkup = null) {
  ...
  if (!res.ok) {
    const errText = await res.text();
    console.error("TELEGRAM SEND MESSAGE ERROR:", errText);
  }
}
...
async function sendPhoto(chatId, photoUrl, caption, env, replyMarkup = null) {
  ...
  if (!res.ok) {
    const errText = await res.text();
    console.error("TELEGRAM SEND PHOTO ERROR:", errText);
  }
}
